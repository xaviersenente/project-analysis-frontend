<template>
  <Block
    ...
    <Infos
      v-if="
        projectData.globalAnalysis?.tagsMissing?.length > 0 ||
        projectData.globalAnalysis?.otherTagsUsage?.br > 10 ||
        projectData.globalAnalysis?.otherTagsUsage?.hr > 8 ||
        Object.keys(projectData.globalAnalysis.obsoleteTagsUsage).length > 0
      ">
      <p v-if="projectData.globalAnalysis?.tagsMissing?.length > 0">
        D'après l'analyse du sujet qui vous a été donné, vous auriez dû utiliser
        les balises ci-dessus pour structurer correctement le contenu&nbsp;!
      </p>
      <p
        v-if="
          projectData.globalAnalysis?.otherTagsUsage?.br > 10 ||
